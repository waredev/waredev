<template>
  <div>
    <div class="w-full h-20 fixed z-50" :class="getBackground()">
      <div class=" lg:max-w-6xl mx-auto px-6 md:px-12 lg:px-20 relative z-10 w-full h-full flex items-center justify-between">
        <nuxt-link to="/">
          <div class="flex items-end">
            <img src="~assets/images/logo.png" class="w-[4.063rem] h-[2.375rem]" />
            <div class="font-nunito font-extrabold text-[#1a2e44] text-2xl ml-2">Waredev</div>
          </div>
        </nuxt-link>
        <div class="hidden xl:flex items-center space-x-12 font-nunito">
          <div class="relative group cursor-pointer">
            <div class='font-nunito group-hover:text-primary flex items-center'>
              <div>Services</div>
              <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 ml-1 transition-all duration-100 rotate-0 group-hover:rotate-180">
    ...
    <div :class="showMenu ? 'w-screen h-screen bg-white px-8 fixed z-20 transition-all' : 'hidden'">
      <div class="font-nunito font-bold mt-32">
        <div class="my-4 cursor-pointer" @click="showSubMenu = !showSubMenu">
          <div class='font-nunito group-hover:text-primary flex items-center'>
            <div>Services</div>
            <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" :class="!showSubMenu ? 'w-5 h-5 ml-1 transition-all duration-100 rotate-0' : 'w-5 h-5 ml-1 transition-all duration-100 rotate-180'">
              <polyline points="6 9 12 15 18 9"></polyline>
            </svg>
          </div>
        ...
        <nuxt-link to="/contact">
          <div @click="closeMenu()" class="text-center my-4 bg-[#1a2e44] px-5 py-1.5 rounded-md text-white font-semibold border-[2px] border-[#1a2e44] hover:bg-transparent hover:text-[#1a2e44] duration-150">
            Let's Talk
          </div>
        </nuxt-link>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      services: [
        ...
        { slug: 'custom-backend-api', label: 'Custom Software Development' },
        // { slug: 'internet-of-things', label: 'Internet of Things (IoT)' },
  ...
  methods: {
    ...
    getBackground() {
      return this.scrollPostion > 80 ? 'backdrop-blur-3xl bg-white/30 custom-drop-blur' : 'bg-transparent';
    },
    closeMenu() {
      this.showMenu = false;
